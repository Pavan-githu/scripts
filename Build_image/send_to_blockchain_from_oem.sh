#!/bin/bash
# =============================================================================
# Build_image/send_to_blockchain_from_oem.sh
#
# OEM approval step — reads current on-chain firmware state, validates that
# approval_stage == 0 (PENDING_OEM), then re-registers with:
#   approved_by_oem  = OEM_IDENTITY env var
#   oem_approved_at  = current ISO-8601 timestamp
#   approval_stage   = 1  (OEM_APPROVED)
#
# Usage:
#   bash scripts/Build_image/send_to_blockchain_from_oem.sh [PROJECT_DIR]
#
...
#   SIGNER_KEY     Ethereum private key of OEM signer (0x...)
#   OEM_IDENTITY   Human-readable OEM approver ID  e.g. "OEM-QA:engineer1"
# =============================================================================
...
[ -n "${OEM_IDENTITY:-}"  ]  || err "OEM_IDENTITY is not set. Add to .env.secrets: export OEM_IDENTITY=\"OEM-QA:engineer1\""
...
echo ""
echo "============================================================"
echo "  OEM Approval — Firmware Blockchain Registration"
echo "============================================================"
log "  Metadata     : $METADATA_JSON"
log "  RPC          : $RPC_URL"
log "  Contract     : $CONTRACT_ADDR"
log "  OEM identity : $OEM_IDENTITY"
echo ""

# ─────────────────────────────────────────────────────────────────────────────
# Run: read on-chain state → validate → approve → register
# ─────────────────────────────────────────────────────────────────────────────
TX_HASH=$(
...
oem_identity  = os.environ["OEM_IDENTITY"]
...
read_calldata = "0x" + (
    read_selector +
    uint256_word(32) +                              # offset to string
    uint256_word(len(ver_bytes)) +                  # string length
    ver_bytes + b"\x00" * ver_pad                   # string data
).hex()
...
if not result_hex or result_hex == "0x":
    ...
    print("       Run send_to_blockchain.sh first to register the firmware.", file=sys.stderr)
    sys.exit(1)
...
# approval_stage must be 0 (PENDING_OEM)
if on_chain["approval_stage"] != 0:
    ...
    print(f"ERROR: Cannot apply OEM approval — current stage is {label} ({on_chain['approval_stage']}).", file=sys.stderr)
    print( "       OEM approval is only valid when approval_stage == 0 (PENDING_OEM).", file=sys.stderr)
    sys.exit(1)

# ── Prepare updated fields ────────────────────────────────────────────────────
oem_approved_at = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
approval_stage  = 1   # OEM_APPROVED

print(f"  Approving as OEM : {oem_identity}", file=sys.stderr)
print(f"  Timestamp        : {oem_approved_at}", file=sys.stderr)
...
params = [
    ...
    ("string",  oem_identity),
    ("string",  oem_approved_at),
    ("string",  on_chain["approved_by_fleet"]),   # unchanged
    ("string",  on_chain["fleet_approved_at"]),   # unchanged
]
...
meta["approved_by_oem"] = oem_identity
meta["oem_approved_at"] = oem_approved_at
...
[ -n "$TX_HASH" ] || err "No tx_hash returned — OEM approval may have failed."

echo ""
ok "OEM approval registered on-chain"
ok "tx_hash        : $TX_HASH"
ok "approval_stage : 1 (OEM_APPROVED)"
ok "approved_by    : $OEM_IDENTITY"
ok "Metadata       : $METADATA_JSON  (updated)"
echo ""
echo "============================================================"
echo "  OEM Approval — COMPLETE"
echo "============================================================"
echo ""
echo "  Next step — Fleet approval:"
echo "    bash $SCRIPT_DIR/send_to_blockchain_from_fleet.sh"
echo ""
